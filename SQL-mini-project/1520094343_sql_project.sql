...
SELECT sub.name, CONCAT( sub.firstname,  " ", sub.surname ) AS member_name, 
      CASE WHEN sub.memid =0 THEN (sub.slots * sub.guestcost)
      ELSE (sub.slots * sub.membercost) END AS cost
FROM (
    SELECT name, firstname, surname, starttime, slots, guestcost, membercost, Bookings.memid
    FROM Bookings
